...
class Worker(QRunnable):
    ...
    def launch_task(self, task):
        if self.threadpool.activeThreadCount() < self.threadpool.maxThreadCount():
            ...
            worker.signals.recording_saved.connect(self.recording_saved_callback)
            worker.signals.dummy_recording_saved.connect(self.recording_dummy_saved_callback)
            worker.signals.feedback_received.connect(self.feedback_received_callback)
            worker.signals.file_sent.connect(self.file_sent_callback)

            self.threadpool.start(worker)
        else:
            self.send_notificaton(f"MAX NUMBER OF THREADS = [{​​​​​self.threadpool.maxThreadCount()}​​​​​].")
